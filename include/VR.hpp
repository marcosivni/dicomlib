...
//is it really worth the hassle involved in using boost date time?
...
//  BOOST_STATIC_ASSERT(sizeof(unsigned long)==4);
	...
	template<> struct TypeFromVR<VR_DA>{typedef	boost::gregorian::date		Type;	};
	...
	template<> struct TypeFromVR<VR_SL>{typedef	signed long					Type;	};
	...
	*/
	...
	{
		BOOST_STATIC_ASSERT(vr!= VR_SQ && vr!=VR_OB && vr!=VR_OW && vr!=VR_UN && vr!=VR_UT);
	};
