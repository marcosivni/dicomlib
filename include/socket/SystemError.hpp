...
public:
	...
	SystemError(std::string Description,int ErrorNumber):Description_(Description),Error_(ErrorNumber)
	{
		...
		int unix_errno=errno;//how is this different?
        int windows_errno=0;//WSAGetLastError();
        std::string s_unix_err="0";//strerror(unix_errno);
        std::string s_win_err="0";//strerror(windows_errno);
