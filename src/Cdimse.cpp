...
namespace dicom
{
	...
	*/
	...
	void HandleCStore(CStoreFunction handler, ServiceBase& pdu, const DataSet& command, const UID& classUID)
	{
		...
		CommandSet::CStoreRSP response(msgID,classUID,Status::SUCCESS);
		pdu.WriteCommand(response,classUID);
	}
	...
	*/
		...
		const DataSet& command, const UID& classUID)
	{
		...
		try
		{
			handler(pdu,command,request_data);//need some way of indicating number of successes/failures?
		}
	/*
		In this case we're handling errors here, but in C-FIND, we don't?  Maybe
		HandleCFind should also have exception handlers.
	*/

		catch(dicom::exception& e)
		{
			cout << e.what();
			CommandSet::CMoveRSP response(msgID,classUID,Status::WARNING,DataSetStatus::NO_DATA_SET);
			//response.set[Completed/failed/warning]()

			pdu.WriteCommand(response,classUID);
			return;
		}
		CommandSet::CMoveRSP response(msgID,classUID,Status::SUCCESS,DataSetStatus::NO_DATA_SET);
		//response.set[Completed/failed/warning]()
		pdu.WriteCommand(response,classUID);

		//idea is that (optionally) we send back 'pending' messages while we're processing
		//command, and a 'completed' message once we're done.
		//Part 4/C.1.4(b)
		//for now, we won't bother with the 'pending' bit, as that would
		// require a lot of cross talk between the library and the server
		//application.
	}
