...
namespace dicom
{
	...
	*/
	bool ClientConnection::InterogateAAssociateAC(AAssociateAC& acknowledgement)
	{
		...
		std::remove_copy_if(acknowledgement.PresContextAccepts_.begin(),	//unfortunately there's no std::copy_if()
			acknowledgement.PresContextAccepts_.end(),
			std::back_inserter(AcceptedPresentationContexts_),
			IsBad);//don't copy elements where result is not 0

		return (AcceptedPresentationContexts_.size()!=0);
	}
	...
	*/
	DataSet ClientConnection::Store(const DataSet& data,TS ts)
	{
		...
		*/
		...
		storeSCU.writeRQ(instUID, data,ts);
		UINT16 status;
		DataSet response;
